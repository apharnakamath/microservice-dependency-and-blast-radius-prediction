...
## What's here
- `services/` — generic OpenTelemetry-instrumented FastAPI service (one process = one microservice, driven by `data/topology.json`)
...
- `scripts/build_ml_dataset.py` — joins failure data + graph features → `data/ml_dataset.csv`
...
python scripts/run_campaign.py            # full sweep, all 18 services x 4 fault variations
...
python3 -m http.server 8000 --directory dashboard   # then open http://127.0.0.1:8000
```
